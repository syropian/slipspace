    ...
    if [ $1 = "deploy" ]; then
      if [ ! -f slipspace.json ]; then
        ...
        if [ ! "$migrate" = "null" ] && [ ! -z $3 ] && [ $3 = "--migrate" ]; then
          ssh -t $user@$host "echo 'Deploying to ${env}...'; cd ${path}; echo 'Fetching source...'; git fetch origin; git reset --hard origin/$env; echo 'Running post-deploy tasks...'; ${after}echo 'Running migrations...'; ${migrate}; exit; bash"
        else
          ssh -t $user@$host "echo 'Deploying to ${env}...'; cd ${path}; echo 'Fetching source...';  git fetch origin; git reset --hard origin/$env; echo 'Running post-deploy tasks...'; ${after}exit; bash"
        fi
